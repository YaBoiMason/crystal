...
uint8_t identify() {
    ...
    if (status & STAT_ERR) {
        ...
        printf("Error reading ATA drive\n");
        return 0;
    }
    ...
    uint8_t buff[256 * 2];
    port_word_ina(ATA_PRIMARY_DATA, buff, 256);
...
void ata_pio_read28(uint32_t LBA, uint8_t sectorcount, uint8_t* target) {
    ...
    for (i = 0; i < sectorcount; i++) {
        ...
        // Transfer the data!
        port_word_ina(ATA_PRIMARY_DATA, (void*)target, 256);
        target += 256;
    }

}
void ata_pio_read_len28(uint32_t LBA, int lenght, uint8_t* target) {
    // HARD CODE MASTER (for now)
    port_byte_out(ATA_PRIMARY_DRIVE_HEAD, 0xE0 | ((LBA >> 24) & 0x0F));
    port_byte_out(ATA_PRIMARY_ERR, 0x00);
    port_byte_out(ATA_PRIMARY_LBA_LO, LBA & 0xFF);
    ...
    // Transfer the data!
    port_word_ina(ATA_PRIMARY_DATA, (void*)target, lenght);
}
...
void ata_pio_write48(uint64_t LBA, uint16_t sectorcount, uint8_t* target) {
    ...
    for (i = 0; i < sectorcount; i++) {
        ...
        // Transfer the data!
        port_word_outa(ATA_PRIMARY_DATA, (void*)target, 256);
        target += 256;
    }
